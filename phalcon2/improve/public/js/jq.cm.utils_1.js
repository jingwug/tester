...
(function($) {
	...
	$.fn.cmtimerUtil = function(method) {
		...
		var methods = {
			init : function(option) {
				return this.each(function() {
					...
					var total_secs = 0;
					total_secs = convertStr2seconds(opt.totalTime);
					var d_begin = opt.beginTime && $.trim(opt.beginTime) ? new moment(opt.beginTime) : new moment(), d_now = opt.serverTime && $.trim(opt.serverTime) ? new moment(opt.serverTime) : new moment();
					if (opt.delay > 0) {
					...
					if (n_b_diff >= 0) {
						...
						//begin到now的时间差减去需要从begin开始及时的时间差,不小于0的话，说明计时已经过了，小于0，表示还需要计时。
		...
		function convertStr2seconds(totalTime) {
			...
			if (len >= 6) {
				console.error('too long,timer walks off the job,大于6位数我不计时 ');
			} else {
				_t = String(1000000 + totalTime);
				var mins = parseInt(_t.substr(-4, 2));
				var seconds = parseInt(_t.substr(-2, 2));
				var hours = parseInt(_t.substr(-6, 2));
				total_secs = countTotalSeconds(seconds, mins, hours);
			}
			return total_secs;
		}

		function countTotalSeconds(sec, min, hour) {
			return sec + min * 60 + hour * 60 * 60;
		}

		function delayToStart(delay, total_secs) {
			var $cont = $(this), opt = $cont.data("cmtimerUtil"), _timer_delay = $cont.data("cmtimerutil_timer_delay");
			if (_timer_delay) {
				clearTimeout(_timer_delay);
			}
			timer_delay = setTimeout(function() {
				beginTimer.call($cont, total_secs);
			}, delay);
			$cont.data("cmtimerutil_timer_delay", timer_delay);
		}
		...
		function beginTimer(total_secs) {
			...
			var $cont = $(this), opt = $cont.data("cmtimerUtil"), _timer = $cont.data("cmtimerutil_timer"), _timer_delay = $cont.data("cmtimerutil_timer_delay");
			if (_timer) {
				clearInterval(_timer);
			} else {
			...
			init : function(option) {
				return this.each(function() {
					...
					function _init(jsondata) {
						$cont.empty();
						var cols = data.cols, rows = data.rows, seats = data.seats;
						var panelCon = "";
					...
					function createSeats(seatData) {
						...
						for (var i = 0; i < len; i++) {
							...
							switch (seatType) {
								...
								case 'N':
									totalCanSelCount++;
									chtml += '<a title="' + titleInfo + '" class="seat ' + opt.canSelected + '" hidefocus="true" data-no="' + seatNo + '" href="javascript:;" data-row=' + rowId + ' data-column=' + columnId + ' data-type="N"></a>';
									break;
								case 'LK':
									chtml += '<a title="' + titleInfo + '" class="seat ' + opt.disable + '" hidefocus="true"    data-no="' + seatNo + '" href="javascript:;"  data-row=' + rowId + ' data-column=' + columnId + ' data-type="LK"></a>';
									break;
								case 'LO':
									totalCanSelCount++;
									chtml += '<a title="' + titleInfo + '" class="seat ' + opt.canSelected + ' ' + opt.lover + '" hidefocus="true" data-no="' + seatNo + '" href="javascript:;" data-row=' + rowId + ' data-column=' + columnId + ' data-type="LO"></a>';
									break;
								default:
									break;
					...
					function _registEvent() {
						$cont.on("click.initCinema", "a.seat", function(ev) {
		...
		var methods = {
			...
			init : function(option) {
				return this.each(function() {
					...
					var _L = false, _R = false, _T = false, _B = false, _H = false, _V = false;
					...
					$(window.document).on("mouseenter", opt.box, function() {
						if (lrBtns && !opt.alwaysShowButton) {
							lrBtns.show();
						}
						if (_timer) {
					...
					$(window.document).on("mouseleave", opt.box, function() {
						if (lrBtns && !opt.alwaysShowButton) {
							lrBtns.hide();
						}
						if (opt.auto) {
